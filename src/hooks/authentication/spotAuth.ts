...
export const spotAuth = () => {
  ...
  const fetchSpotifyCode = useCallback(
    async (code: string) => {
      ...
      try {
        ...
        if (accessToken) {
          ...
          const expTime = expiryTime
            .setMinutes(expiryTime.getMinutes() + expiresIn / 60)
            .toString();

          setItem("SpotifyAccessToken", accessToken);
          setItem("SpotifyRefreshToken", refreshToken);
          setItem("SpotifyTokenExpiry", expTime);
  ...
  useEffect(() => {
    const refreshTime = getItem("refreshTime");
    if (refreshTime) {
      setRefreshTime(refreshTime);
    }
  }, [getItem, setRefreshTime]);
