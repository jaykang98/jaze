...
export const useAuthenticator = () => {
    ...
    const fetchSession = async (token: string) => {
        const apiSigString = `api_key${process.env.REACT_APP_APIKEY}methodauth.getSessiontoken${token}${process.env.REACT_APP_SECRETKEY}`;
        const apiSig = CryptoJS.MD5(apiSigString).toString();
        const sessionUrl = `https://ws.audioscrobbler.com/2.0/?method=auth.getSession&token=${process.env.REACT_APP_SECRETKEY}&api_key=${process.env.REACT_APP_APIKEY}&api_sig=${apiSig}&format=json`;
