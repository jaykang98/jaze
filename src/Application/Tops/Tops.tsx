...
const Tops: React.FC<ActivityConstructorProps> = ({ userID }) => {
    ...
    const renderItemContent = (data: any, type: 'artist' | 'album' | 'track', isTable: boolean = false) => {
        const dataType = type === 'artist' ? data?.topartists : type === 'album' ? data?.topalbums : data?.toptracks;
        const items = dataType[type].slice(0, isTable ? 10 : 1);

        return isTable ? (
            <DisplayTable data={items.map((item: any, index: number) => ([
                `${index + 1}`,
                item.name,
    ...
    const renderUserInfo = () => {
        ...
        const dataForDisplay: (string | JSX.Element)[][] = [
            ...
            [<FontAwesomeIcon key="playcount-icon" icon={faMusic} />, "Playcount", formatNumber(user.playcount)],
        ];
